...
int hand_type(hand hand) {
    char found_cards[HANDSIZE];
    int found_cards_counts[HANDSIZE];
    int total_found_ranks = 0;
    int max_rank_count = 0;
    int most_common_non_j_type_i;
    int j_count = 0;
    for (int i = 0; i < HANDSIZE; i++) {
        ...
        for (int j = 0; j < total_found_ranks; j++) {
            if (found_cards[j] == hand[i]) {
                found_cards_counts[j] ++;
                if (found_cards_counts[j] > max_rank_count) {
                    max_rank_count = found_cards_counts[j];
                    most_common_non_j_type_i = j;
                }
                was_found = 1;
                break;
            }
        ...
        if (!was_found) {
            found_cards[total_found_ranks] = hand[i];
            found_cards_counts[total_found_ranks] = 1;
            if (max_rank_count < 1) {
                max_rank_count = 1;
                most_common_non_j_type_i = total_found_ranks;
            }
            total_found_ranks++;
        }
    }
    ...
    found_cards_counts[most_common_non_j_type_i] += j_count;
    max_rank_count += j_count;
